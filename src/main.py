...
bot = Bot(command_prefix="?", help_command=commands.DefaultHelpCommand(
    no_category="Commands"))


# finds initial timestamps for summoners in the boost_check database
def most_recent_timestamp_finder(account_id):
    initial_JSON = requests.get("https://na1.api.riotgames.com/lol/match/v4/matchlists/by-account/" +
                                account_id + "?queue=420&season=13&api_key=" + config.RIOT_API_KEY)
...
def boosted_score_calculator(game_match_data_JSON, participants_index, team_ID):
    ...
    for k in range(10):
        if game_match_data_JSON.json()["participants"][k]["teamId"] != team_ID:
            ...
            team_total_kills += game_match_data_JSON.json(
            )["participants"][k]["stats"]["kills"]
    ...
    for i in range(10):
        ...
        if game_match_data_JSON.json()["participants"][i]["teamId"] != team_ID or i == participants_index:
            ...
            team_kp_sum += ((game_match_data_JSON.json()["participants"][i]["stats"]["kills"] +
                            game_match_data_JSON.json()["participants"][i]["stats"]["assists"])/team_total_kills)

    # calculates average kp of the summoner's teammates, excluding the summoner in question
    team_kp_avg = team_kp_sum / 4

    player_kp = ((game_match_data_JSON.json()["participants"][participants_index]["stats"]["kills"] +
                 game_match_data_JSON.json()["participants"][participants_index]["stats"]["assists"])/team_total_kills)
    ...
    summoner_gold = game_match_data_JSON.json(
    )["participants"][participants_index]["stats"]["goldEarned"]

    # finding the total gold of the summoner's lane opponent
    summonner_role = game_match_data_JSON.json(
    )["participants"][participants_index]["timeline"]["role"]
    summoner_lane = game_match_data_JSON.json(
    )["participants"][participants_index]["timeline"]["lane"]
    ...
    summoner_opponent_gold = game_match_data_JSON.json(
    )["participants"][opponent_index]["stats"]["goldEarned"]
    # finds how the summoner's gold compares to lane opponent, gives +/- score dependent on less/more gold
    boosted_score_gold_diff_component = (
        (summoner_opponent_gold / summoner_gold) - 1) * 50
...
async def recent_game_loop(boosted_bot_channel):
    while True:
        ...
        for k in range(database_list_length):
            ...
            game_idJSON = requests.get("https://na1.api.riotgames.com/lol/match/v4/matchlists/by-account/" + str(
                database_list[k][1]) + "?queue=420&season=13&api_key=" + config.RIOT_API_KEY)
            try:
                ...
                if game_idJSON.json()["matches"][0]["timestamp"] > database_list[k][4]:
                    ...
                    cursor.execute("UPDATE boost_check SET timestamp = ? WHERE summoner_name = ?", [
                                   game_idJSON.json()["matches"][0]["timestamp"], database_list[k][0]])
                    db.commit()

                    new_game_id = game_idJSON.json()["matches"][0]["gameId"]
                    new_game_champion_id = game_idJSON.json()[
                        "matches"][0]["champion"]
                    new_game_match_dataJSON = requests.get(
                        "https://na1.api.riotgames.com/lol/match/v4/matches/" + str(new_game_id) + "?api_key=" + config.RIOT_API_KEY)
                    ...
                    for i in range(10):
                        if new_game_match_dataJSON.json()["participants"][i]["championId"] == new_game_champion_id:
                            if new_game_match_dataJSON.json()["participants"][i]["stats"]["win"] == True:
                                team_ID = new_game_match_dataJSON.json()[
                                    "participants"][i]["teamId"]
                                boosted_score = boosted_score_calculator(
                                    new_game_match_dataJSON, i, team_ID)

                                # finds data regarding current boosted score and number of games analyzed
                                # connects to the boost_check database
                                db = sqlite3.connect(BOOST_CHECKER_DIR_PATH)
                                cursor = db.cursor()
                                cursor.execute("SELECT current_boosted_score from boost_check where summoner_name = ?", [
                                               database_list[k][0]])
                                current_boosted_score = cursor.fetchone()
                                cursor.execute("SELECT number_of_games_analyzed from boost_check where summoner_name = ?", [
                                               database_list[k][0]])
                                number_of_games = cursor.fetchone()
                                # calculates updated versions of this data
                                new_number_of_games = number_of_games[0] + 1
                                new_boosted_score = round(
                                    ((current_boosted_score[0] * number_of_games[0]) + boosted_score) / (new_number_of_games), 2)

                                # updates the database
                                cursor.execute("UPDATE boost_check SET current_boosted_score = ? where summoner_name = ?", [
                                               new_boosted_score, database_list[k][0]])
                                cursor.execute("UPDATE boost_check SET number_of_games_analyzed = ? where summoner_name = ?", [
                                               new_number_of_games, database_list[k][0]])
                                db.commit()
                                db.close()
                            ...
                            print(
                                "successfully detected and analyzed a summoner in boost_check")
                            break

            except Exception as e:
                if type(e) == KeyError:
                    print("Standard GET request error where index 'matches' cannot be found in returned JSON. Happens occasionally, nothing to worry about")

                elif type(e) == UnboundLocalError:
                    await boosted_bot_channel.send("Detected a new game has been played by " + database_list[k][0] + ", but error with Riot's API in 'match/v4/matchlists' has occured")
...
async def ranked_stats(ctx, *, league_name):
    try:
        idJSON = requests.get("https://na1.api.riotgames.com/lol/summoner/v4/summoners/by-name/" +
                              league_name + "?api_key=" + config.RIOT_API_KEY)
        summoner_id = idJSON.json()["id"]
        ranked_dataJSON = requests.get(
            "https://na1.api.riotgames.com/lol/league/v4/entries/by-summoner/" + summoner_id + "?api_key=" + config.RIOT_API_KEY)
        ...
        win_percentage = str(
            round((float(wins) / (float(wins) + float(losses)) * 100), 2))
...
async def boosted_list_adder(ctx, *, league_name):
    ...
    cursor.execute(
        "SELECT * FROM boost_check WHERE summoner_name = ?", [league_name])
    if cursor.fetchone() == None:
        ...
        cursor.execute("INSERT INTO boost_check VALUES (?, ?, ?, ?, ?)", [
                       league_name, account_id, 0, 0, timestamp])
        db.commit()
        db.close()
...
async def boosted_list_remover(ctx, *, league_name):
    ...
    if cursor.fetchone() == None:
        ...
        cursor.execute(
            "DELETE FROM boost_check WHERE summoner_name = ?", [league_name])
        db.commit()
        db.close()
...
async def boosted_leaderboard(ctx):
    ...
    if cursor.fetchall() == []:
        ...
        cursor.execute(
            "SELECT summoner_name, number_of_games_analyzed, current_boosted_score FROM boost_check ORDER BY current_boosted_score DESC")
        ...
        embed.add_field(name="Summoner Name",
                        value=summoner_name_section, inline=True)

        # makes number of games analyzed column
        number_of_games_analyzed_section = ""
        for k in range(len(leaderboard_list)):
            number_of_games_analyzed_section += str(
                leaderboard_list[k][1]) + "\n"

        embed.add_field(name="# of Games Analyzed",
                        value=number_of_games_analyzed_section, inline=True)
        ...
        embed.add_field(name="Average Boosted Score",
                        value=boosted_score_section, inline=True)
...
async def on_ready():
    ...
    for k in range(database_list_length):
        ...
        cursor.execute("UPDATE boost_check SET timestamp = ? WHERE summoner_name = ?",
                       (updated_timestamp, database_list[k][0]))
        db.commit()
